...
class IDSController(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.model    = load_model("modeles/MLP.h5")
        ...
        self.feature_names = numeric_cols + ohe_names
    ...
    def on_features(self, features):
        ...
        # 1) log the raw features
        #fv = df.to_dict(orient="records")[0]
        #self.logger.debug(f"[IDS] feature vector: {fv}")

        # 2) get probabilities instead of just `.predict()`
        probs = self.model.predict(df)[0]
        classes = self.lb.classes_
        #prob_dict = dict(zip(classes, probs))
        #self.logger.debug(f"[IDS] prediction probs: {prob_dict}")

        # 3) pick the top label
